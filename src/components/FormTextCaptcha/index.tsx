...
export type FormTextCaptchaProps = {
  ...
  onVerified?: () => void | undefined;             // 验证通过后的回调函数
};
...
const FormTextCaptcha: React.FC<FormTextCaptchaProps> = (props) => {
  const { placeholder, rules, form, onVerified, ...restProps } = props;
  ...
  const [captchaImgSrc, setCaptchaImgSrc]: [string, any] = useState('');
  const [captchaLoading, setCaptchaLoading]: [boolean, any] = useState(false);
  const [captchaStatus, setcaptchaStatus]: [CaptchaStatus, any] = useState(CaptchaStatus.empty);
    ...
    <div>
      <Row>
        <Col span={14}>
          ...
          >
            <Input size="large" maxLength={5} value={value}
              onChange={(e: any) => {
